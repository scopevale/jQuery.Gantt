...
// adapted from jquery.fn.gantt.js
            ...
            */
            create: function (element) {
                ...
                */
                if (typeof settings.source !== "string") {
                    ...
                    try { // test for valid JSON
                       var jsdata = jQuery.parseJSON( settings.source )
                       element.data = jsdata;    
                    } catch(e) { //non JSON may be URL
            ...
            init: function (element) {
                ...
                if (settings.periodStart) {
                    element.dateStart = tools.dateDeserialize(settings.periodStart);
                } else {
                    element.dateStart = tools.getMinDate(element);
                }
                if (settings.periodEnd) {
                    element.dateEnd = tools.dateDeserialize(settings.periodEnd)
                } else {
            ...
            rightPanel: function (element, leftPanel) {
                ...
                if (settings.shiftDate) {
                    var today = new Date(settings.shiftDate);
                } else {
                    var today = new Date();
                }
                    ...
                    case "hours":
                        ...
                        for (var i = 0; i < range.length; i++) {
                            ...
                            */
                            ...
                            if (rfy !== year && !settings.hoursOnly) {
                                yearArr.push(
                            ...
                            */
                            ...
                            if (rm !== month && !settings.hoursOnly) {
                                monthArr.push(
                            ...
                            */
                            ...
                            if (rgetDay !== getDay && !settings.hoursOnly) {
            ...
            navigation: function (element) {
                ...
                } else if (settings.navigate === "page") {
                    /* Navigation panel */
                    ganttNavigate = $('<div class="navigate" />')
                        .append($('<span role="button" class="nav-link nav-page-back"/>')
                            .html('&lt;')
            ...
            fillData: function (element, datapanel, leftpanel) {
                ...
                $.each(element.data, function (i, entry) {
                    if (i >= element.pageNum * settings.itemsPerPage && i < (element.pageNum * settings.itemsPerPage + settings.itemsPerPage)) {
                        ...
                        $.each(entry.shifts, function (j, day) {
                            var _bar = null;
                            switch (settings.scale) {
                                case "hours":
...
//                var maxDate = null;
                var maxDate = new Date(1354360490);

                $.each(element.data, function (i, entry) {
                    if (entry.shifts) {
                        $.each(entry.shifts, function (i, date) {
            ...
            getMinDate: function (element) {
                //var minDate = null;
                var minDate = new Date(1349090090);
            ...
            dateDeserialize: function (dateStr) {
                ...
                var date = new Date(date.getUTCFullYear(), date.getUTCMonth(), date.getUTCDate(), date.getUTCHours(), date.getUTCMinutes());
        ...
        this.each(function () {
            ...
            */
            ...
            if (settings.hoursOnly) {
                settings.scale = 'hours';
                settings.maxScale = 'hours';
                settings.minScale = 'hours';
            }
